...
## Section ↔ task cross-reference (generated by `plan-to-tasks` 2026-06-10)

Sprint tag `voice-int`, shipping build **0.3.0**. Execution `seq` is **not** plan-section order:
the IO-cog code (§2) and its blink (§5) land together, then the observability harness (§4) proves
"do we hear commands" before the larger `robot_dog_top` dispatch-loop reshape (§3); docs (§6) follow
final code; verification (§7) is the gate.

| Plan § | Deliverable                                   | Task   | seq |
| ------ | --------------------------------------------- | ------ | --- |
| §1     | Voice driver OBJ → `isp_i2c` (bus 2)          | «#20»  | 1   |
| §2     | IO cog owns bus 2 + recognized-CMDID telemetry| «#21»  | 2   |
| §5     | Minimal green-LED blink on recognition        | «#22»  | 3   |
| §4     | `test_voice.spin2` observability harness      | «#23»  | 4   |
| §3     | `robot_dog_top` dispatch loop + gating skel   | «#24»  | 5   |
| §6     | Docs: wiring / pin tables / spec / theory-ops | «#25»  | 6   |
| §7     | Verification: compile gate + bench playbook   | «#26»  | 7   |
