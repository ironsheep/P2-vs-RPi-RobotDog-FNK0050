...
- **[ELEC]** — resolve the open electrical item (bus-2 pull-up source + supply rail, wiring §3a).
...
## Exercise 1 — Bus-2 electrical pre-flight: pull-up source + supply rail [ELEC] [NEW]

- **Verifies:** wiring §3a — **resolves the open electrical item** that sets the `voice.start()`
  pull-up argument. Do this **before** trusting recognition; it can force a one-line code change +
  recompile that all later exercises depend on.
...
- **Setup:** P2 powered, DF2301Q wired on P18 (SCL) / P16 (SDA) and powered from its supply rail.
  Flash `test_voice` (Exercise 2's build) so the P2 configures the bus-2 pins (`PU_NONE` as shipped),
  then leave it idling at the `LISTENING` banner.
- **Action / OBSERVE (multimeter, DC):**
  ...
  2. **Pull-up source** — with the bus idle (no transfer), measure **SDA (P16) and SCL (P18) idle
     voltage to GND:**
     - **Idle ≈ module Vcc (lines pulled high):** the **board carries its own pull-ups** → keep
       `VOICE_I2C_PULLUP = PU_NONE` (=0). ✅ as shipped.
     - **Idle ≈ 0 V / floating-low:** the board does **not** pull up → set
       `isp_io_controller.VOICE_I2C_PULLUP = 1` (`PU_1K5`), **recompile** (re-run Ex 0), and re-flash
       before Exercise 2. (The weaker `PU_3K3` can idle the bus low — don't use it. Vendor USER-GUIDE.)
- **PASS TELL:** you have a **recorded decision**: supply rail = ____ V; SDA/SCL idle = ____ V →
  `VOICE_I2C_PULLUP` = **PU_NONE / PU_1K5** (circle one). Code constant now matches the meter.
- **Feeds closeout:** this is the wiring-doc metering deliverable — reconcile wiring §3a + the
  `VOICE_I2C_PULLUP` comment to the measured value.
- **Pass/fail:** `[ ]`  rail ____ V · idle ____ V · pull-up arg = ________ · code reconciled? ______
...
- **Verifies:** §4 `«#23»` (the primary observability gate) + §5 `«#22»` (green blink). **The headline
  exercise of the sprint.** Depends on Ex 0 green and Ex 1's pull-up decision applied.
...
- **Ex 1 metering result** (rail + pull-up arg) → reconcile wiring §3a and the `VOICE_I2C_PULLUP`
  comment; this **resolves the open electrical item**.
