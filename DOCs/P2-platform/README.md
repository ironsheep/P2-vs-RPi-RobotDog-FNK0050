# P2-platform — the physical platform (build + wiring + CAD)

![Doc Type](https://img.shields.io/badge/doc-hardware%20platform-blue)
...
Everything about the **physical swap** — dropping a **Propeller 2 Edge Module + breakout
board** into the spot where the Raspberry Pi normally sits on the Robot Dog connection
board. This folder holds the build walk-through, the electrical wiring map, and the
adapter-plate CAD:

- **[`HARDWARE_SETUP.md`](HARDWARE_SETUP.md)** — photo walk-through: model + print the
  adapter plate, assemble the P2 board onto it, mount it on the robot.
- **[`P2_MIGRATION_WIRING.md`](P2_MIGRATION_WIRING.md)** — the electrical side of the swap:
...
- **Adapter-plate CAD** — the editable plate models (see *Files* below).

The rest of this README is the **CAD story** — how the adapter plate was modeled and printed.
...
|------|--------|------------|
| `HARDWARE_SETUP.md` | doc | Build walk-through — model/print the plate, assemble the P2 board, mount on the robot. |
| `P2_MIGRATION_WIRING.md` | doc | Electrical swap — power architecture, header→P2 wiring map, 3.3 V-vs-5 V hazards. |
